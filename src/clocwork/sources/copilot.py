...
HELD = "per-call rows missing from the store, so not read from shutdown snapshots"
...
def scan(repo, homes):
    ...
    snapshots. What earlier runs archived for it stays, since the archive
    keeps the larger record per day -- unless that day's record from a
    later run outgrows it, for the archive keeps records per day and not
    per session; a session whose rows were gone before any run saw it is
